const elements = {
  ...
  const settings = {
    targetToy: null,
    collectedNumber: 0,
    maxPicks: 1, // Add a maxPicks property
  }
  ...
  class Toy extends WorldObject {
    ...
    collectToy(toy) {
      ...
      if (settings.collectedNumber >= settings.maxPicks) {
        ...
        congratsText.style.fontSize = 'clamp(2rem, 5vw, 3rem)';
        congratsText.style.fontWeight = 'bold';
        congratsText.style.color = 'gold';
        congratsText.style.textShadow = '0 0 10px rgba(255, 215, 0, 0.5)';
        congratsText.style.padding = '1rem';
        congratsText.style.zIndex = '1000';
        congratsText.style.textAlign = 'center';
        document.body.appendChild(congratsText);

        // Remove the text after a few seconds and show the alert
        setTimeout(() => {
            congratsText.remove(); // Remove the congratulations text
            if (settings.collectedNumber >= settings.maxPicks) {
            const userConfirmed = confirm('You have collected enough toys! Do you want to go to the special page?');
            if (userConfirmed) {
              window.location.href = 'flower.html'; // Redirect to the special page
            }
            }
